...
!  define physics buffer indicies here
  integer :: &
    ...
    w_down_in_cl_idx, & ! CLUBB mean in-cloud downdraft speed
    cld_updr_frac_idx, & ! CLUBB cloudy updraft fraction
    cld_downdr_frac_idx, & ! CLUBB cloudy downdraft fraction
    pblh_idx, &         ! PBL pbuf
...
!-------------------------------------------------------------------------------
    ...
    call pbuf_add_field('w_down_in_cl',    'global', dtype_r8, (/pcols, pverp/),             w_down_in_cl_idx)
    call pbuf_add_field('cld_updr_frac',   'global', dtype_r8, (/pcols, pverp/),             cld_updr_frac_idx)
    call pbuf_add_field('cld_downdr_frac', 'global', dtype_r8, (/pcols, pverp/),             cld_downdr_frac_idx)
...
!    use parameters_tunable, only: clubb_param_readnl ! Brian: should be added to the api
    ...
    namelist /clubbpbl_diff_nl/ clubb_cloudtop_cooling, clubb_rainevap_turb, clubb_expldiff, &
                                ...
                                clubb_l_enable_relaxed_clipping
    namelist /clubb_param_nl/ clubb_C1, clubb_C1b, clubb_C1c, clubb_C2rt, clubb_C2thl, &
                              clubb_C2rtthl, clubb_C4, clubb_C_uu_shr, clubb_C_uu_buoy, &
                              clubb_C6rt, clubb_C6rtb, clubb_C6rtc, clubb_C6thl, &
                              clubb_C6thlb, clubb_C6thlc, clubb_C7, clubb_C7b, &
                              clubb_C8, clubb_C8b, clubb_C11, clubb_C11b, clubb_C11c, &
                              clubb_C14, clubb_C_wp2_pr_dfsn, clubb_C_wp3_pr_tp, &
                              clubb_C_wp3_pr_turb, clubb_C_wp3_pr_dfsn, clubb_beta, &
                              clubb_gamma_coef, clubb_gamma_coefb, clubb_gamma_coefc, &
                              clubb_pdf_component_stdev_factor_w, clubb_mu, &
                              clubb_c_K1, clubb_nu1, clubb_c_K2, clubb_nu2, clubb_c_K8, &
                              clubb_nu8, clubb_c_K9, clubb_nu9, clubb_c_K10, clubb_c_K10h, &
                              clubb_c_K_hmb, clubb_wpxp_L_thresh, clubb_lmin_coef, &
                              clubb_mult_coef, clubb_Skw_denom_coef, clubb_up2_sfc_coef, &
                              clubb_Skw_max_mag, clubb_C_invrs_tau_bkgnd, clubb_C_invrs_tau_sfc, &
                              clubb_C_invrs_tau_shear, clubb_C_invrs_tau_N2, &
                              clubb_C_invrs_tau_N2_wp2, clubb_C_invrs_tau_N2_xp2, &
                              clubb_C_invrs_tau_N2_wpxp, clubb_C_invrs_tau_N2_clear_wp3, &
                              clubb_C_invrs_tau_wpxp_Ri, clubb_C_invrs_tau_wpxp_N2_thresh, &
                              clubb_C_wp2_splat, clubb_xp3_coef_base, clubb_xp3_coef_slope, &
                              clubb_altitude_threshold, clubb_rtp2_clip_coef, clubb_Cx_min, &
                              clubb_Cx_max, clubb_Richardson_num_min, clubb_Richardson_num_max, &
                              clubb_a3_coef_min
...
!$OMP END PARALLEL
    ...
    call addfld ('W_DOWN_IN_CL_CLUBB', (/ 'lev' /),  'A',    'm/s', 'Mean in-cloud downdraft velocity')
    call addfld ('CLD_UPDR_FRAC_CLUBB', (/ 'lev' /),  'A',     '-', 'Cloudy updraft fraction')
    call addfld ('CLD_DOWNDR_FRAC_CLUBB', (/ 'lev' /),  'A',   '-', 'Cloudy downdraft fraction')
...
!PMA gustiness output fields
    ...
    if (history_clubb) then
       ...
       call add_default('W_DOWN_IN_CL_CLUBB', 1, ' ')
       call add_default('CLD_UPDR_FRAC_CLUBB', 1, ' ')
       call add_default('CLD_DOWNDR_FRAC_CLUBB', 1, ' ')
...
!-------------------------------------------------------------------------------
   ...
   real(core_rknd) :: w_down_in_cloud_out(pcols,pverp)
   real(core_rknd) :: cloudy_updraft_frac_out(pcols,pverp)
   real(core_rknd) :: cloudy_downdraft_frac_out(pcols,pverp)
   ...
   real(r8), pointer, dimension(:,:) :: w_down_in_cloud ! Mean in-cloud downdraft velocity      [m/s]
   real(r8), pointer, dimension(:,:) :: cloudy_updraft_frac   ! Cloudy updraft fraction         [-]
   real(r8), pointer, dimension(:,:) :: cloudy_downdraft_frac ! Cloudy downdraft fraction       [-]
...
! numbers are coefficients of the empirical equation
   ...
   call pbuf_get_field(pbuf, w_down_in_cl_idx, w_down_in_cloud)
   call pbuf_get_field(pbuf, cld_updr_frac_idx, cloudy_updraft_frac)
   call pbuf_get_field(pbuf, cld_downdr_frac_idx, cloudy_downdraft_frac)
   ...
   call t_startf('adv_clubb_core_col_loop')
    ...
    do t=1,nadv    ! do needed number of "sub" timesteps for each CAM step
      ...
      call advance_clubb_core_api( gr, pverp, ncol,                                                  & ! intent(in)
           ...
           wprcp_out(:ncol,:), w_up_in_cloud_out(:ncol,:), w_down_in_cloud_out(:ncol,:),                    & ! intent(out)
           cloudy_updraft_frac_out(:ncol,:), cloudy_downdraft_frac_out(:ncol,:),                            & ! intent(out)
    ...
    do k=1,pverp
      do i = 1, ncol
        ...
        w_down_in_cloud(i,k) = real(w_down_in_cloud_out(i,pverp-k+1), kind = r8)
        cloudy_updraft_frac(i,k) = real(cloudy_updraft_frac_out(i,pverp-k+1), kind = r8)
        cloudy_downdraft_frac(i,k) = real(cloudy_downdraft_frac_out(i,pverp-k+1), kind = r8)
...
!PMA adds gustiness and tpert
   ...
   call outfld( 'W_DOWN_IN_CL_CLUBB', w_down_in_cloud,       pcols, lchnk )
   call outfld( 'CLD_UPDR_FRAC_CLUBB', cloudy_updraft_frac,  pcols, lchnk )
   call outfld( 'CLD_DOWNDR_FRAC_CLUBB', cloudy_downdraft_frac, pcols, lchnk )
...
!-------------------------------------------------------------------------------
    ...
    if (first_call) then
      ...
      l_mono_flux_lim_spikefix = .false. ! Needs to be false to recover old code flow.
